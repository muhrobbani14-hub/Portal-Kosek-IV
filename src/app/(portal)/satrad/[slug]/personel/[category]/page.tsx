...
import { getPersonnelCategory } from "@/lib/personnel-data";
import {
  getUnitPersonnelDefaultRows,
  getUnitPersonnelFileName,
} from "@/lib/unit-personnel-data";
...
type UnitPersonnelCategoryPageProps = {
  params: Promise<{
    slug: string;
    category: string;
  }>;
...
export default async function UnitPersonnelCategoryPage({
  params,
}: UnitPersonnelCategoryPageProps) {
  ...
  const tableKey = getUnitPersonnelTableKey(unit.slug, category.slug);
  const defaultRows: EditableTableDefaultRow[] = getUnitPersonnelDefaultRows(
    unit.slug,
    category.slug,
  );
  const fileName = getUnitPersonnelFileName(unit.slug, category.slug);
    ...
    <main className="relative min-h-[calc(100vh-5rem)] overflow-hidden bg-[#050b18]">
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-5rem)] w-full max-w-[1600px] flex-col px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-wrap justify-end gap-3">
          ...
          <Link
            href={`/satrad/${unit.slug}/personel`}
        ...
        <section className="relative flex-1 overflow-hidden rounded-[6px] border border-white/10 bg-[#061225]/80 shadow-[0_28px_80px_rgba(0,0,0,0.45)] backdrop-blur-md">
          ...
          <div className="border-b border-white/10 px-5 py-4 sm:px-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.22em] text-yellow-200/90">
                  {unit.name}
                </p>
                <h1 className="mt-2 text-2xl font-black uppercase tracking-[0.08em] text-white sm:text-3xl">
                  Data Personel {category.title}
                </h1>
              </div>
                ...
                {rows.length ? (
                  ...
                  <tr>
                    <td
                      ...
                      className="px-6 py-12 text-center text-sm font-semibold uppercase tracking-[0.16em] text-slate-300"
                    >
