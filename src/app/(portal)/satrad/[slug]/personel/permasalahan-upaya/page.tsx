...
type UnitPersonnelProblemActionPageProps = {
  params: Promise<{
    slug: string;
  }>;
};

function getTableKey(unitSlug: string) {
  return `unit:${unitSlug}:personel:permasalahan-upaya`;
}

export default async function UnitPersonnelProblemActionPage({
  params,
}: UnitPersonnelProblemActionPageProps) {
  ...
  const tableKey = getTableKey(unit.slug);
        ...
        <section className="relative overflow-hidden rounded-[6px] border border-white/10 bg-[#07162d]/25 px-4 py-8 shadow-[0_28px_80px_rgba(0,0,0,0.45)] backdrop-blur-[2px] sm:px-8">
          ...
          <div className="relative z-10">
            <div className="mx-auto mb-8 w-fit min-w-[260px] rounded-[6px] border border-white/25 bg-white/35 px-8 py-5 text-center shadow-[0_18px_45px_rgba(0,0,0,0.28)] backdrop-blur-md sm:min-w-[460px]">
              ...
              <h1 className="mt-2 text-2xl font-black uppercase tracking-[0.08em] text-white drop-shadow sm:text-3xl">
